...
class mod_member extends inc_mod_default {
	...
	function get_order_list() {
		...
		if(count($arr_day) > 0 ) {
			...
			$arr_x = array();
			$obj_result = $obj_db->select("select order_addtime,order_id,order_ids,order_day,order_total,order_score_money,order_favorable,order_comment,order_total_pay,order_name,order_sex,order_detail,order_state,order_shop_id,order_addprice from " . cls_config::DB_PRE . "meal_order" . $str_where . " order by order_id desc" . $arr_return['pageinfo']['limit']);
			while($obj_rs = $obj_db->fetch_array($obj_result)) {
	...
	function get_order_detail($order_id) {
		...
		$str_where = " where order_id='" . $order_id . "' and order_user_id='" . cls_obj::get("cls_user")->uid . "'";
		$obj_rs = $obj_db->get_one("select * from " . cls_config::DB_PRE . "meal_order" . $str_where);
		$shopid = 0;
		if(!empty($obj_rs)) {
		...
		//取店铺送餐时间
		$obj_shopinfo = $obj_db->get_one("select shop_extend,shop_addprice from " . cls_config::DB_PRE . "meal_shop where shop_id='" . $shopid . "'");
