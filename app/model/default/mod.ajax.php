...
class mod_ajax extends inc_mod_default {
	...
	function save_order(){
		...
		//查看店铺运营信息
		$obj_shop = $obj_db->get_one("select shop_name,shop_state,shop_user_id,shop_sms,shop_sms_tel,shop_extend,shop_support_score,shop_addprice from " . cls_config::DB_PRE . "meal_shop where shop_id='" . $shop_id . "'");
		if(empty($obj_shop)) {
		...
		$arr_order=array(
			...
			"order_addprice" => $obj_shop['shop_addprice'],
		);
		...
		if(!empty($neworder_user_tips) && fun_is::tel($arr_order['order_mobile'])) {
			$site_title = cls_config::get("site_title" , "sys");
