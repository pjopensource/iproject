...
class mod_act_gift extends inc_mod_default {
	function get_gift_all() {
		...
		$obj_result = $obj_db->select("select * from " . cls_config::DB_PRE . "act_gift" . $where);
	...
	function on_exchange() {
		...
		if(!empty($obj_gift['gift_total_day'])) {
			$daynum = $obj_gift['gift_num_today'];
			if( fun_is::isdate($obj_gift['gift_time']) && strtotime($obj_gift['gift_time']) < strtotime(date("Y-m-d 00:00:00"))) $daynum = 0;
			if( $daynum >= $obj_gift['gift_total_day'] ) return array("code" => 500 , "msg" => "【" . $obj_gift['gift_name'] . "】限制每天只能兑换" . $obj_gift['gift_total_day'] . "份，今天已兑完，明天再来吧");
		}
		if(!empty($obj_gift['gift_user_num'])) {
			...
			if(!empty($obj_user_num) && $obj_user_num['num'] >= $obj_gift['gift_user_num']) return array("【" . $obj_gift['gift_name'] . "】限制每人只能兑换" . $obj_gift['gift_user_num'] . "份，您已兑完，换个礼品吧");
		}
		//每天开放时间段，检测
		...
		if($arr_msg['code']!=0) {
			...
			return array("code"=> 500 , "msg" => "兑换礼品失败");
		}
		//扣减用户积分
