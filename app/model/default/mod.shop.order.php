...
class mod_shop_order extends inc_mod_shop {
	...
	function on_state() {
		...
		$arr = tab_meal_order::on_state($str_id , $state , $beta , "order_state=0 and order_shop_id='" . $this->shop_info['id'] . "'");
	...
	function on_award() {
		...
		if(!empty($arr_id)) $str_id = $arr_id; //优先考虑 arr_id
		$arr = tab_meal_order::on_award($str_id,"order_shop_id='" . $this->shop_info['id'] . "'");
		if($arr['code'] != 0) {
			$arr_return['code'] = $arr['code'];
			$arr_return['msg']  = $arr['msg'];
		}
	...
	function on_delete() {
		...
		if(!empty($arr_id)) $str_id = $arr_id; //优先考虑 arr_id
		$arr = tab_meal_order::on_delete($str_id,"order_shop_id='" . $this->shop_info['id'] . "'");
		if($arr['code'] != 0) {
			$arr_return['code'] = $arr['code'];
			$arr_return['msg']  = $arr['msg'];
		}
