	...
	function get_pagelist($is_del = 0) {
		...
		if( $arr_search_key['key'] != '' ) {
			if(cls_config::USER_CENTER=='user.klkkdj') {
				$arr_where_s[] = "(user_name like '%" . $arr_search_key['key'] . "%' or user_email like '%" . $arr_search_key['key'] . "%' or user_netname like '%" . $arr_search_key['key'] . "%')";
			} else {
				$arr_x = cls_obj::get("cls_user")->get_user($arr_uid,false);
