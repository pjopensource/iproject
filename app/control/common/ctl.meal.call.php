...
class ctl_meal_call extends mod_meal_call {
	...
	function act_default() {
		...
		if(!cls_obj::get("cls_user")->is_admin() ) {
			...
			$area_pid = cls_config::get("area_city_id" , "meal");
			$arr_area = array('所有地区');
	...
	function act_print_test() {
		...
		$print_source = array(
			...
			"{收款信息}"=>"应收：38",
			"{菜品列表}"=>"<table><tr><td>名称</td><td>数量</td><td>单价</td></tr><tr><td>套餐一</td><td>1</td><td>10</td></tr><tr><td>套餐二</td><td>1</td><td>13</td></tr><tr><td>套餐三</td><td>1</td><td>15</td></tr></table>",
			chr(10)=>"<br>",
			" "=>"&nbsp;"
		);
	...
	function act_print() {
		...
		$shop_info = $this->get_shop_info($shop_id);
		$print_temp = fun_get::filter($shop_info["shop_printinfo"],true);
		$tel = $tel_mobile = '';
