...
class tab_meal_order {
	...
	static function get_perms($key) {
		if( empty(self::$perms) ) {
			self::$perms = array(
				"state" => array("已接受" => 1 , "等待处理" => 0 , "已拒绝" => -1 , "待付款" => -2 ,"过期未付款" => -3 ,"过期已取消" => -4 ,"用户取消" => -5),
	...
	function on_state($arr_id , $state , $beta = '' , $msg_where = '') {
		...
		if($state == 1 && $arr['code'] == 0) {
			...
			$obj_result = cls_obj::db_w()->select("select order_user_id,order_shop_id from " . cls_config::DB_PRE . "meal_order" . $where);
			while($obj_rs = cls_obj::db_w()->fetch_array($obj_result)) {
				tab_meal_order::on_refresh_user($obj_rs['order_user_id']);
				//同步店铺销售量
				cls_obj::db_w()->on_exe("update " . cls_config::DB_PRE . "meal_shop set shop_sold=shop_sold+1 where shop_id='" . $obj_rs['order_shop_id'] . "'");
			}
		}
		return $arr;
	}
