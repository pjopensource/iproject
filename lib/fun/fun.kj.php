...
class fun_kj{
	...
	static function get_ads($id) {
		$obj_db = cls_obj::db();
		$obj_rs = $obj_db->get_one("select ads_html from " . cls_config::DB_PRE . "other_ads where ads_id='" . $id . "' and ads_state>0 and ads_starttime<'" . TIME . "' and ads_endtime>'" . TIME . "'");
		if(!empty($obj_rs)) {
	...
	//获取地区html列表
	static function get_area($pid = -1 , $depth = -1) {
		if($pid < 0 ) {
		...
		$str_where = "area_depth<=" . $depth;
		if($pid>0) $str_where .= " and " . cls_db::concat("," , "area_pids" , ",") . " like '%," . $pid . ",%'";
		$obj_result = $obj_db->query("select area_id,area_name,area_pid,area_depth,area_val from " . cls_config::DB_PRE . "sys_area where " . $str_where . " order by area_depth,area_sort,area_name");
