...
class com_sms {
	...
	function on_send( $arr ) {
		...
		//是否开启短信
		$state = (int)cls_config::get("state","sms");
		if(!$state) return array("code"=>500,"msg"=>"未开启发送短信功能(后台短信配置)");
		//如果为测试环境，只允许测试手机号
		...
		$arr = tab_other_sms::on_save($arr_fields);
		return $arr_return;
	}
	...
	function get_recont() {
		$count = cls_config::get("count_id" , "sms");
		$pwd = cls_config::get("count_pwd" , "sms");
		$arr = array("code"=>0 , "cont"=>"||18665921015#119#2012-08-16 13:53:46#||18665921015#B#2012-08-16 13:53:58#");
