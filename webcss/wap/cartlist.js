var jscartlist = new function() {
	...
	this.refresh_price = function(shopid , objrow) {
		...
		if(obj) {
			if(kj.toint(total)>kj.toint('{$user_repayment}')) {
				obj.disabled = true;
	...
	this.chk_info = function() {
			...
			if(document.frm_main.louhao1.value=="具体位置")  document.frm_main.louhao1.value="";
			if(document.frm_main.mobile.value=="手机号码")  document.frm_main.mobile.value="";
			if(document.frm_main.name.value=="收件人")  document.frm_main.name.value="";
	...
	//改变数量
	this.change_num = function(id , num ,setval) {
		...
		if(setval) obj_cart_num.value = setval;
