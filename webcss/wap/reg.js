var jsreg = new function() {
	...
	this.show_verify = function() {
		...
		kj.dialog({'html':this.verifycode_html,'id':'verifycodereg','type':'html','title':'请输入验证码','w':280,'showbtnmax':false,'showbtnhide':false});
		this.verify_refresh();
	}
	//显示邮箱验证框
			...
			if(obj_data.isnull) {
				...
				if(obj_data.code == 0) {
					...
					if(url) {
						...
						window.open(kj.cfg("baseurl") + "/index.php?app=member" , "_self");
					}
				} else {
					if('msg' in obj_data && obj_data.msg!='') {
						alert(obj_data.msg);
