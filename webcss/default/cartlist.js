var jscartlist = new function() {
	...
	this.refresh_price = function(shopid , objrow) {
		...
		if(obj) {
			if(kj.toint(total)>this.user_repayment) {
				obj.disabled = true;
	...
	this.changearea = function(val , index , defautval) {
		...
		if(val!='') {
			var key2 = key;
			for(i = 0; i <10 ;i++) {
				if(key2 in this.areainfo && kj.toint(this.areainfo[key2]["dispatch_price"])>0) {
					this.minleast = this.areainfo[key2]["dispatch_price"];
					break;
				}
				if(key2 in this.areainfo && 'area_pid' in this.areainfo[key2] && this.areainfo[key2]["area_pid"]) {
					key2 = "id_" + this.areainfo[key2]["area_pid"];
				} else {
